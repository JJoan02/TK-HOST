...
let handler = async (m, { conn, usedPrefix, command, args, isOwner, isAdmin, isROwner }) => {
    ...
    switch (type) {
        ...
        default:
            if (!/[01]/.test(command)) {
                return conn.reply(m.chat, `🤖 *Opciones disponibles:*\n\n${generarMenu()}\n\n📌 Usa *${usedPrefix}on <opción>* para activar o *${usedPrefix}off <opción>* para desactivar.`, m);
            }
            throw false;
    }

    await conn.reply(m.chat, `✅ *Configuración actualizada:*\n\n✦ Opción: *${type.toUpperCase()}*\n✦ Estado: *${isEnable ? 'ACTIVADO' : 'DESACTIVADO'}*\n✦ Aplicado: ${isAll ? 'GLOBAL' : isUser ? 'USUARIO' : 'GRUPO'}`, m);
};
