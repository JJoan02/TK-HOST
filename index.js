...
const validatePhoneNumber = async () => {
  let phoneNumber;
  while (true) {
    phoneNumber = await question('📞 Ingresa tu número de WhatsApp (ejemplo: 51912345678): ');
    if (/^\d{10,15}$/.test(phoneNumber)) break;
    console.log(
      chalk.red('❌ Número inválido. Asegúrate de no incluir "+" y que tenga entre 10 y 15 dígitos.')
    );
  }
  return phoneNumber;
};

// === Filtrar ciertos mensajes de consola ===
const filterStrings = [
